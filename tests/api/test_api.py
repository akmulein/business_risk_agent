...
from counterparty_verification.analysis.service import AnalysisService
from counterparty_verification.api import create_app
from counterparty_verification.chat.models import ChatAgentResult, ChatSource
from counterparty_verification.chat.questions import QuestionService
from counterparty_verification.chat.service import ChatService
...
def app(repository):
    ...
    application = create_app(settings, repository=repository)
...
async def test_validation_and_not_found(app) -> None:
    async with AsyncClient(
        ...
        missing_preview = await client.get("/api/v1/counterparties/1234567894/preview")
