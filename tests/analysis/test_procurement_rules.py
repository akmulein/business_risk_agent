...
from counterparty_verification.analysis.analyzers import analyze_procurement
from counterparty_verification.analysis.rules.procurement import CHECKS, build_view
from counterparty_verification.domain import CounterpartyCard, Observation, RiskLevel

RULES_DOC = Path(__file__).parents[2] / "docs" / "rules" / "analyze_procurement.md"
...
def test_contract_signing_gap_is_observed() -> None:
    ...
    assert "расхождение 2" in detail
    assert "агрегированные счётчики" in detail
    assert "уклон" not in detail.lower()
    assert "недобросовест" not in detail.lower()
...
# -- Added for the tools eval/regression effort: YearAggregate uses
# `int(x or 0)` / `float(x or 0)`, which means an explicit 0 and a missing
# (None) count are indistinguishable once aggregated -- pinning this as a
# deliberate, known behavior rather than an accident someone trips over
# later.


def test_explicit_zero_and_missing_count_produce_the_same_aggregate() -> None:
    card_zero = _card(
        procurements=[_procurement(year=2025, tender_admitted_count=0)]
    )
    card_missing = _card(
        procurements=[_procurement(year=2025, tender_admitted_count=None)]
    )

    aggregate_zero = build_view(card_zero).by_year[2025]
    aggregate_missing = build_view(card_missing).by_year[2025]

    assert aggregate_zero.admitted == aggregate_missing.admitted == 0
