...
def test_sole_trader_does_not_produce_ownership_findings() -> None:
    card = _card(
        [
            _item(
                "activity", 0, role="main", code="70.22", description="Консультирование"
            )
        ],
        inn="120705254031",
...
def test_permit_is_matched_by_the_main_activity_only() -> None:
    ...
    with_license = CounterpartyCard.model_validate(
        {
            ...
            "structure_items": [
                _director(),
                _item(
                    "activity", 0, role="main", code="86.21", name="Врачебная практика"
                ),
            ],
            "legal_events": [
                {
    ...
    detail = _observed(main_needs_permit)["license_gap"].detail
    ...
    assert "разрешения нет" not in detail.lower()
    assert "лицензии нет" not in detail.lower()
    assert "нарушение" not in detail.lower()
