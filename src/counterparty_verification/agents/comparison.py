...
from counterparty_verification.agents.provider import openrouter_provider
...
def _facts(companies: list[ComparisonCompany]) -> list[dict[str, Any]]:
    ...
    for company in companies:
        ...
        if company.active_enforcements is not None and company.active_enforcements > 0:
            add(
                company,
                "active_enforcements",
                "concern",
                f"У {name} — {_enforcement_text(company.active_enforcements)}.",
            )
...
class ComparisonAgent:
    ...
    def _build_summary(
        ...
        for statement in ordered_statements:
            ...
            if unknown_refs:
                raise RuntimeError(
                    "Указаны неизвестные fact_refs: " + ", ".join(sorted(unknown_refs))
                )
            _check_numbers(statement, facts_by_id)
        for statement in output.company_statements:
            expected_prefix = f"{statement.company_inn}."
            if not all(ref.startswith(expected_prefix) for ref in statement.fact_refs):
                raise RuntimeError(
                    "Утверждение компании ссылается на факты другого контрагента"
                )
        summary = " ".join(statement.text.strip() for statement in ordered_statements)
        if len(summary) > 1200:
