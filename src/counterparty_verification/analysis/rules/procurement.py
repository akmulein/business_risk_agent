...
Five checks, described in `docs/rules/analyze_procurement.md` under the same
...
from counterparty_verification.analysis.rules.structure import as_date
...
class IndexedProcurement:
    ...
    are validated against the card by `agents.question_answer.flatten_field_paths`.
    """
...
def _observed(
    code: str, title: str, detail: str, evidence: list[Evidence]
) -> Observation:
...
def _check_contract_signing_gap(view: ProcurementView) -> Observation | None:
    """Расхождение агрегированных счётчиков побед и контрактов."""
    gaps = sorted(
    ...
    return _observed(
        ...
        "Расхождение между победами и заключёнными контрактами",
        f"В данных количество отражённых побед превышает количество "
        f"отражённых заключённых контрактов: {years_text} "
        f"(расхождение {total_gap}). Это агрегированные счётчики; без связи "
        "с конкретной закупкой нельзя делать вывод о конкретном действии "
        "поставщика. Причину расхождения стоит уточнить у контрагента.",
        evidence,
    )
