...
mongo_args="--host ${MONGO_HOST} --port ${MONGO_INTERNAL_PORT}"
...
# shellcheck disable=SC2086
mongosh ${mongo_args} --quiet /seed/scripts/build_read_model.js

# Index creation is idempotent and runs after the read model is replaced.
# shellcheck disable=SC2086
mongosh ${mongo_args} --quiet /seed/scripts/create_indexes.js

# shellcheck disable=SC2086
document_count="$(mongosh ${mongo_args} --quiet --eval \
  'db.getSiblingDB(process.env.MONGO_DATABASE).getCollection(process.env.MONGO_COLLECTION).countDocuments({})')"

# shellcheck disable=SC2086
card_count="$(mongosh ${mongo_args} --quiet --eval \
