...
Ten checks, described in `app/analysis/rules/specs/analyze_structure.md` under the same names
...
def _check_director_change(view: StructureView) -> Observation | None:
    ...
    if days is None or days < 0 or days > RECENT_APPOINTMENT_DAYS:
        return None
    recent = (
        " Это меньше трёх месяцев до даты отчёта." if days <= RECENT_CHANGE_DAYS else ""
    )
    return _observed(
        "director_change",
...
def _check_ownership_integrity(view: StructureView) -> Observation | None:
    ...
    amounts = [entry for entry in view.active_founders if entry.item.amount is not None]
...
def _check_ownership_chain(view: StructureView) -> Observation | None:
    ...
    if view.parents:
        ...
        reasons.append(
            f"функции руководителя исполняет управляющая организация ({names})"
        )
