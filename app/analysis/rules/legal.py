...
Five checks, described in `app/analysis/rules/specs/analyze_legal.md` under the same names
...
from app.analysis.rules.structure import (
    LICENSE_INACTIVE_MARKERS,
    as_date,
    as_float,
)
...
class LegalView:
    ...
    def status_bucket(self, role: str, case_status: str) -> StatusBucket:
        ...
        amounts = [
            as_float(row.item.amount) for row in rows if row.item.amount is not None
        ]
...
def _passive_amount(pending: StatusBucket, appealed: StatusBucket) -> float | None:
    parts = [value for value in (pending.amount, appealed.amount) if value is not None]
    if not parts:
        return None
    return sum(parts)
...
def _format_status_side(
    label: str, pending: StatusBucket, appealed: StatusBucket, finished: StatusBucket
) -> str:
    return (
...
def _yearly_totals(
    ...
    for year, items in grouped.items():
        ...
        amounts = [
            as_float(item.item.amount) for item in items if item.item.amount is not None
        ]
        amount = sum(amounts) if amounts else None
...
def _check_arbitration_trend(view: LegalView) -> Observation | None:
    ...
    doubled = _grew(float(last_count), float(prev_count)) or _grew(
...
def _check_enforcement_load(view: LegalView) -> Observation | None:
    ...
    if not active:
        return None
    amounts = [
        as_float(row.item.amount) for row in active if row.item.amount is not None
    ]
    total = sum(amounts) if amounts else None
    identifiers = [row.item.external_id for row in active if row.item.external_id][
        :MAX_EVENT_IDS
    ]
    dates = [
    ...
    return _observed(
        ...
        f"{date_text} Неактивные записи не учитываются. Это фактор "
        f"исполнительного риска; сумма и статус приведены так, как они "
        f"отражены в отчёте.{extra}",
        evidence,
    )
...
def _check_inspection_findings(view: LegalView) -> Observation | None:
    ...
    notable = [
        row for row in view.inspections if _inspection_is_notable(row.item.status)
    ]
    if not notable:
        return None
    ...
    more = (
        f" Всего таких проверок: {len(notable)}." if len(notable) > len(shown) else ""
    )
    evidence = [
        evidence
        for row in shown
...
def _check_license_validity(view: LegalView) -> Observation | None:
    ...
    if expiring:
        ...
        parts.append(f"истекает в ближайшие {LICENSE_EXPIRING_DAYS} дней: {names}")
