...
Five checks, described in `app/analysis/rules/specs/analyze_finance.md` under the same names
...
from app.analysis.rules.structure import as_float
...
class IndexedFinance:
    ...
    which are validated against the card by `agents.question_answer.flatten_field_paths`.
    """
...
CHECKS: tuple[FinanceCheck, ...] = (
    ...
    FinanceCheck("revenue_decline", "Снижение выручки", _check_revenue_decline),
    FinanceCheck(
        "liquidity_gap",
        "Покрытие краткосрочных обязательств",
        _check_liquidity_gap,
    ),
    FinanceCheck("negative_equity", "Отрицательный капитал", _check_negative_equity),
