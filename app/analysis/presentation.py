...
def _sentences(value: str) -> list[str]:
    return [
        part.strip() for part in SENTENCE_BOUNDARY.split(value.strip()) if part.strip()
    ]
        ...
        if chapter_name == "reputation":
            rows.append(
                FactorSummaryItem(
                    ...
                    status=(RiskLevel.HIGH if negative_reputation else RiskLevel.LOW),
                    details=(
                        negative_reputation
...
def build_visualization_data(card: CounterpartyCard) -> VisualizationData:
    ...
    years = range(min(legal_by_year), max(legal_by_year) + 1) if legal_by_year else []
    legal_dynamics = [
        LegalChartPoint(
            year=year,
